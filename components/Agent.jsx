...
export default function AgentComplet() {
  ...
  const generateLeadsBatch = async (batchCount) => {
    const prompt = `Tu es un agent de génération de leads pour batterie-stockage.fr, spécialiste en batteries de stockage d'énergie solaire LFP (marque Renon Power).

Génère EXACTEMENT ${batchCount} leads fictifs mais très réalistes en région ${region}. Mix installateurs RGE et particuliers avec panneaux solaires existants.

IMPORTANT : Tu DOIS retourner EXACTEMENT ${batchCount} objets JSON. Pas moins, pas plus.

Pour chaque lead, génère des données 100% différentes des autres (noms, emails, téléphones, besoins uniques) :
- nom : nom complet réaliste français
- email : email professionnel réaliste (domaines variés : gmail, orange, wanadoo, entreprise.fr, etc.)
- tel : numéro français réaliste (format 0X XX XX XX XX)
- region : "${region}"
- type : "Installateur RGE" ou "Particulier"
- score : nombre entre 0 et 100
- statut : "Chaud" si score > 70, "Tiède" si score entre 40 et 70, "Froid" si score < 40
- besoin : 1 phrase unique décrivant son besoin précis en stockage batterie
- puissance_kw : puissance installation solaire en kW (nombre entier)

Réponds UNIQUEMENT avec un tableau JSON valide contenant exactement ${batchCount} objets. Aucun texte avant ou après, aucun backtick.`;
  ...
  const generateLeads = async () => {
    ...
    const messages = [
      { msg: "🔍 Analyse du marché solaire...", p: 10 },
      { msg: "📡 Identification des prospects...", p: 30 },
      { msg: "🧠 Scoring IA en cours...", p: 60 },
    ...
    try {
      ...
      for (let b = 0; b < totalBatches; b++) {
        ...
        setLoadingMsg(`🧠 Génération lot ${b + 1}/${totalBatches} (${batchCount} leads)...`);
        setProgress(Math.round(((b + 1) / totalBatches) * 90));
        const batch = await generateLeadsBatch(batchCount);
        allLeads = [...allLeads, ...batch];
      }

      const unique = allLeads.filter((l, i, arr) =>
        arr.findIndex(x => x.email === l.email) === i
      );
  ...
  const generateEmails = async () => {
    ...
    for (let i = 0; i < selected.length; i++) {
      ...
      const prompt = `Tu es un expert en vente de batteries de stockage pour batterie-stockage.fr (batteries LFP Renon Power).
...
Mentionne batterie-stockage.fr, la marque Renon Power, la garantie 10 ans, le stock France et la livraison 48-72h.
Signe avec "L'équipe batterie-stockage.fr — contact@batterie-stockage.fr".

Réponds UNIQUEMENT avec un JSON : {"sujet": "...", "corps": "..."}
Sans texte ni backticks autour.`;
  ...
  const exportCSV = () => {
    const selected = leads.filter((_, i) => selectedLeads.includes(i));
    const rows = [
      "EMAIL,PRENOM,NOM,REGION,TYPE,SCORE,STATUT,BESOIN,PUISSANCE_KW,SUJET_EMAIL,CORPS_EMAIL",
      ...selected.map(l => {
        ...
        return `${l.email},${prenom},${nom},${l.region},${l.type},${l.score},${l.statut},"${l.besoin}",${l.puissance_kw},"${sujet}","${corps}"`;
      })
    ].join("\n");
    ...
    a.href = url;
    a.download = `campagne_batterie_stockage_${region.replace(/\s/g, "_")}_${selected.length}leads.csv`;
    a.click();
    setExported(true);
    setStep("export");
  };
  ...
  return (
    <div style={{
      ...
      color: "#E8E8F0", padding: "32px 28px", maxWidth: 900, margin: "0 auto"
    }}>
      ...
      <div style={{ marginBottom: 36 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
          <div style={{
            ...
            background: "#6C63FF", boxShadow: "0 0 10px #6C63FF", animation: "blink 2s infinite"
          }} />
        ...
        {STEPS.map((s, i) => (
          <div key={s.id} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center" }}>
            <div style={{
              ...
              boxShadow: i === stepIndex ? "0 0 20px #6C63FF55" : "none", transition: "all 0.4s"
            }}>
              {i < stepIndex ? "✓" : s.icon}
            </div>
            <span style={{ fontSize: 10, color: i <= stepIndex ? "#E8E8F0" : "#333", textAlign: "center", letterSpacing: 0.5 }}>
              {s.label}
            </span>
            {i < STEPS.length - 1 && (
              <div style={{
                position: "absolute", left: `${(i + 1) * 25}%`, top: 17,
                width: "25%", height: 2,
      ...
      {step === "config" && !loading && (
        <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid #1a1a2e", borderRadius: 16, padding: 28 }}>
          <h2 style={{ fontSize: 14, color: "#666", letterSpacing: 3, textTransform: "uppercase", marginBottom: 24 }}>
            Configuration
          </h2>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
            <div>
              <label style={{ fontSize: 11, color: "#444", letterSpacing: 2, display: "block", marginBottom: 8 }}>RÉGION CIBLE</label>
              <select value={region} onChange={e => setRegion(e.target.value)} style={{
                width: "100%", padding: "10px 14px", borderRadius: 8,
                border: "1px solid #1a1a2e", background: "#0d0d1a", color: "#fff", fontSize: 13, fontFamily: "inherit"
              }}>
          ...
          <div style={{
            ...
            {count > 15 && <span style={{ color: "#6C63FF", display: "block", marginTop: 6 }}>⚡ {Math.ceil(count / 15)} lots de génération — comptez ~{Math.ceil(count / 15) * 8} secondes.</span>}
          </div>
          <button onClick={generateLeads} style={{
            ...
            ▶ Lancer la génération ({count} leads)
          </button>
        </div>
      )}
      ...
      {step === "leads" && !loading && (
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <div>
              <h2 style={{ fontSize: 14, color: "#666", letterSpacing: 3, textTransform: "uppercase", margin: 0 }}>Leads générés</h2>
              <p style={{ fontSize: 12, color: "#444", margin: "4px 0 0" }}>{selectedLeads.length}/{leads.length} sélectionnés</p>
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={toggleAll} style={{
                padding: "8px 14px", borderRadius: 8, border: "1px solid #222",
                background: "transparent", color: "#555", fontSize: 11, cursor: "pointer", fontFamily: "inherit"
              }}>
                {selectedLeads.length === leads.length ? "Tout désélect." : "Tout sélect."}
              </button>
              <button onClick={() => setStep("config")} style={{
                padding: "8px 16px", borderRadius: 8, border: "1px solid #222",
                background: "transparent", color: "#555", fontSize: 12, cursor: "pointer", fontFamily: "inherit"
              }}>← Retour</button>
      ...
      {step === "emails" && !loading && (
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            <div>
              <h2 style={{ fontSize: 14, color: "#666", letterSpacing: 3, textTransform: "uppercase", margin: 0 }}>Emails personnalisés</h2>
              <p style={{ fontSize: 12, color: "#444", margin: "4px 0 0" }}>{Object.keys(emails).length} emails rédigés par l'IA</p>
            </div>
            <button onClick={exportCSV} style={{
              padding: "10px 24px", borderRadius: 8, border: "none",
              background: "linear-gradient(135deg, #00C896, #00A878)",
              color: "#fff", fontSize: 13, cursor: "pointer", fontFamily: "inherit", fontWeight: 800, letterSpacing: 1
            }}>⬇ Exporter CSV Brevo</button>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {leads.filter((_, i) => selectedLeads.includes(i)).map((lead, i) => {
              ...
              return (
                <div key={i} style={{ background: "rgba(255,255,255,0.02)", border: "1px solid #1a1a2e", borderRadius: 10, overflow: "hidden" }}>
                  <div onClick={() => setExpandedEmail(isOpen ? null : lead.email)} style={{
                    padding: "14px 18px", display: "grid", gridTemplateColumns: "1fr auto auto",
                    alignItems: "center", gap: 14, cursor: "pointer"
                  }}>
                    <div>
                      <div style={{ fontWeight: 700, fontSize: 13 }}>{lead.nom}</div>
                      {email
                        ? <div style={{ fontSize: 11, color: "#6C63FF", marginTop: 2 }}>📧 {email.sujet}</div>
                        : <div style={{ fontSize: 11, color: "#444" }}>⏳ En cours...</div>
                      }
                    </div>
                    <Tag label={lead.statut} color={statusColors[lead.statut]} />
                  ...
                  {isOpen && email && (
                    <div style={{ padding: "0 18px 18px", borderTop: "1px solid #1a1a2e" }}>
                      <div style={{ background: "#0d0d1a", borderRadius: 8, padding: 16, marginTop: 14 }}>
                        ...
                        <div style={{ fontSize: 12, color: "#aaa", lineHeight: 1.8, whiteSpace: "pre-wrap", borderTop: "1px solid #1a1a2e", paddingTop: 12 }}>
                          {email.corps}
                        </div>
                      </div>
                    </div>
        ...
        <div>
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <span style={{ fontSize: 11, color: "#444",letteSpacing: 2 }}>CONTENU CSV</span>
              <button onClick={() => {
          ...
          <button onClick={() => {
            setStep("config"); setLeads([]); setEmails({});
            setExported(false); setCsvContent(""); setSelectedLeads([]);
          }} style={{
